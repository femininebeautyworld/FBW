...
<script type="text/javascript">
	function ajaxRequest() {
			...
			if (mygetrequest.readyState == 4) {
				if (mygetrequest.status == 200
					...
					for (var i = 0; i < rssentries.length; i++) {
						if( i % 5 == 0)
						{
						output += '<div class="galcolumn" id="item'+(i+1)+'MAwUE" style="width: 48.0867%; padding-left: 30px; padding-bottom: 30px; float: left; box-sizing: border-box;">';
						}
						output += '<div style="margin-bottom: 30px; opacity: 1;">';
						if( rssentries[i].postPictureUrl != null )
							...
							output += '<span class="stalac_box_img"> <a href="'+host+'/viewPaxDetailPost.do?method=displayPost&postId='+rssentries[i].postId+'"><img width="'+rssentries[i].imageWidth+'" height="'+rssentries[i].imageHeight+'" alt="img2" class="attachment-large wp-post-image" src="'+rssentries[i].postPictureUrl+'" style="width: 100%; height: auto; display: block; margin-left: auto; margin-right: auto;"></a> </span> <h3> <a href="'+host+'/viewPaxDetailPost.do?method=displayPost&postId='+rssentries[i].postId+'">'+rssentries[i].postTitle+'</a></h3>';
								...
								{
									output += '<p class="item stalac_box">'+rssentries[i].postDescription+'....</p>';
								}
                               }
						else
							{
					...
					output += '</div>';
					if( (i != 0 && ( (i+1) % 5 == 0 ) ) ||  (i+1) == rssentries.length )
					{
						output += '</div>';
					}
					}
	    ...
	    mygetrequest.open("GET", "<%=RequestUtils.getBaseURI(request)%>/homePage.do?method=fetchResultsByPostType&callType='ajax'",true);
		mygetrequest.send();
	}
</script>
...
<div id="home_cont">
	<div id="stalac_cont" class="gridalicious">
		...
		<c:forEach var="post" items="${posts}" varStatus="loop">
			<c:if test="${loop.index % 4 eq 0}">
				<div class="galcolumn" id="item${count}AhA9R" style="width: 32.5865073437889%; padding-left: 30px; padding-bottom: 30px; float: left; box-sizing: border-box;">
			</c:if>
	...
	<button id="button" type="button" onclick="ajaxRequest()"></button>
